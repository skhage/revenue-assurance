...
(speaker notes, timing, objection handling); the `.html` file is the presentable, self-contained
20-slide deck built from that narrative (open directly in a browser — no build step); the `.pptx` is
a portable export of the same slides for offline use, generated by `scripts/build_pptx.py`.
