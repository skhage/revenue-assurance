...
| Pipeline health gate                                      | Agent Workbench (all 4 tabs)                        | `dq_audit.status`, `observed_at` freshness vs. 72h threshold                                                                                                                                                            | Blocks Investigation/Prioritization/Recovery with a destructive alert when RED, unavailable, **or stale** — stale evidence is a hard block, not a soft warning                             |
...
| Tab                  | Source                                                                                                          | Notes                                                                                                                                                                                                                               |
| :------------------- | :-------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pipeline reliability | `cdm_tmforum.revenue_assurance.dq_audit` via new `GET /api/dq/audit` (inline SQL, no named query — see ADR-015) | Summarized server-side (`server/routes/dqAudit.ts`, `summarizePipelineHealth`) into `unavailable`/`red`/`stale`/`ok`; freshness threshold 72h; any unrecognized/malformed/inconsistent DQ status is treated as failing, never GREEN |
...
- **Pipeline gate:** if `dq_audit` is `RED`, unreachable, **or `stale`**, the Investigate/Prioritize/Recovery tabs render only a destructive "Blocked by Pipeline Reliability agent" alert — no recommendation is computed, no Apply button renders. Stale evidence is treated as a hard block, not a soft warning: recommending against evidence that might be out of date is exactly the failure mode this gate exists to prevent. Freshness is evaluated **per required (`INLINE`) check** — each check's own latest-timestamped row is compared to the 72h threshold independently, so one fresh check can never mask a different check that is stale or missing a timestamp (`summarizePipelineHealth` in `server/routes/dqAudit.ts`). `isBlocked()` in `client/src/lib/agents/types.ts` is the single source of truth for the block decision — every panel calls it rather than re-deriving the condition.
...
- **Apply: assign (Prioritize & route):** `POST /api/cases/:id/assign` to the recommended analyst, preceded by the structured `[Agent: …]` note (note-before-mutation ordering).
- **Apply: move to Recovering (Recovery playbook):** walks the existing `New → Investigating → Recovering` transition guard (assigning first if needed) via `POST /api/cases/:id/assign` then `POST /api/cases/:id/status`, with the structured note written first. No new mutation route is introduced anywhere in this tab.
- **Durable note idempotency:** every "Apply" tags its note with a per-approved-run key (`agent:<slug>:<exception_id>:<run_id>`). Browser storage retains the key plus the exact approved note until the dependent mutation succeeds: exact retries after a lost response, remount, or reload reuse both, while a materially changed recommendation/note obtains a distinct key. Lakebase enforces at most one note per `(exception_id, idempotency_key)` via a unique partial index, and the route uses one conditional `INSERT ... ON CONFLICT` statement to atomically dedupe the same body or reject a different body with `409`; two simultaneous mismatched requests cannot both pass.
- **Guaranteed selected-exception visibility (Prioritize & route):** the ranked table merges the shared `selected` exception into its scored set even if the default 100-row batch never returned it, and always renders it — pinned below the visible top 20 with a "(rank #N, outside top 20)" label if its score would otherwise place it further down, without displacing the top-20 ordering itself.

### States & copy

- **Loading:** `LoadingRegion` + skeletons, matching Queue/Cases.
- **Blocked (red/unavailable/stale):** `Alert variant="destructive"` — "Blocked by Pipeline Reliability agent" + the specific DQ failure or staleness reason. Stale is a hard block, same as red/unavailable.
...
- **Recovery disclosure:** the Recovery Playbook's expected-recovery %, owner role, and deadline are template assumptions, not measured outcomes — labeled with a `Demo data` badge and an inline caveat, not just "Deterministic."
