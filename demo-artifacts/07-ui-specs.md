...
- **Pipeline gate:** if `dq_audit` is `RED`, unreachable, **or `stale`** (freshest observation > 72h old), the Investigate/Prioritize/Recovery tabs render only a destructive "Blocked by Pipeline Reliability agent" alert — no recommendation is computed, no Apply button renders. Stale evidence is treated as a hard block, not a soft warning: recommending against evidence that might be out of date is exactly the failure mode this gate exists to prevent. `isBlocked()` in `client/src/lib/agents/types.ts` is the single source of truth for this — every panel calls it rather than re-deriving the block condition.
