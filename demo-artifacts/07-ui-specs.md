...
- **Durable note idempotency:** every "Apply" tags its note with a stable key (`agent:<slug>:<exception_id>`) and Lakebase enforces at most one note per `(exception_id, idempotency_key)` via a unique partial index on `ra.case_notes` — a Postgres-level guarantee, not a client-side flag, so a retry is always safe even after a lost response, a component remount (e.g. re-selecting the same exception), or a full page reload.
